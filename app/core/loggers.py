...
def _get_file_handler(filename: str) -> logging.Handler:
    ...
    if log_path in _file_handlers:
        return _file_handlers[log_path]
...
def get_logger(
    ...
    *,
    ...
    # 전체 로그 파일 핸들러
    logger.addHandler(_get_file_handler("global"))
...
def setup_library_loggers(debug: bool = False):
    """
    Uvicorn, FastAPI, SQLAlchemy 등 외부 라이브러리의 로거 설정을 덮어씁니다.
    """
    # Uvicorn 로거 설정
    # uvicorn.error는 표준 출력, uvicorn.access는 액세스 로그
    for logger_name in ["uvicorn", "uvicorn.access", "uvicorn.error", "fastapi"]:
        get_logger(logger_name, filename="server", debug=debug)

    # SQLAlchemy 로거 설정
    # sqlalchemy.engine은 쿼리 로그
    get_logger(
        "sqlalchemy.engine",
        filename="database",
        debug=debug,
    ...
    get_logger("sqlalchemy.pool", filename="database", debug=debug)
