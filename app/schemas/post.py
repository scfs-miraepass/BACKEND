...
class Posts(Post, table=True):
    content: Optional["PostContent"] = Relationship(
        back_populates="post", sa_relationship_kwargs={"uselist": False}, passive_deletes=True
    )
...
class PostContent(SQLModel, table=True):
    ...
    post_id: int = Field(
        sa_column=Column(Integer, ForeignKey("post.id", ondelete="CASCADE"), primary_key=True),
